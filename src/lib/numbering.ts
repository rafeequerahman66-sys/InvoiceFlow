 ...
 * scoped per organization. MUST be called inside a transaction so concurrent
 * saves can't collide. Returns e.g. "INV/2026-27/0007".
 */
export async function nextNumber(
  ...
  const seq = await tx.numberSequence.upsert({
    where: { orgId_docType_fyLabel: { orgId, docType, fyLabel } },
    create: { orgId, docType, fyLabel, lastNumber: 1 },
